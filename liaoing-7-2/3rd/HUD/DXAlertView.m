   ...
   rightButtonTitle:(NSString *)rigthTitle
{
    if (self = [super init]) {
        ...
        self.progressView.frame = CGRectMake((kAlertWidth - contentLabelWidth) * 0.5, CGRectGetMaxY(self.alertTitleLabel.frame)+30, contentLabelWidth, 2);
        [self addSubview:self.progressView];
        
        self.alertContentLabel = [[UILabel alloc] initWithFrame:CGRectMake((kAlertWidth - contentLabelWidth) * 0.5, CGRectGetMaxY(self.progressView.frame), contentLabelWidth, 25)];
        self.alertContentLabel.numberOfLines = 1;
        self.alertContentLabel.textAlignment = NSTextAlignmentRight;
...
- (void)changeTitle:(NSString *)title andProgessValue:(float)value
{
    self.alertTitleLabel.text = title;
    self.progressView.progress = value;
    self.alertContentLabel.text = [NSString stringWithFormat:@"%.2f%%",value*100];
}
